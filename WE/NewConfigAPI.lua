...
-- @tab [ o p t ] cnfg a configuration table that may contain these fields:
--
...
function config.read(file, cnfg)
    ...
    while line do
        if line:find('^%[') then
            ...
            if i1 and not ignore_assign then
...
--End of Penlight's config.lua


local cfg = { name = "cfg", config = {}, comments = {} }

function cfg.add(section, name, value)
    cfg.config[section] = cfg.config[section] or {}
    cfg.config[section][name] = value
end

function cfg.remove(section, name)
    if cfg.config[section] and cfg.config[section][name] ~= nil then
        cfg.config[section][name] = nil
    end
end

function cfg.hasValue(section, name)
    return cfg.config[section] and cfg.config[section][name]~=nil
end

function cfg.addComment(section, name, comment)
    cfg.comments[section] = cfg.comments[section] or {}
    cfg.comments[section][name] = comment
end

function cfg.removeComment(section, name)
    if cfg.comments[section] and cfg.comments[section][name] then
        cfg.comments[section][name] = nil
    end
end

function cfg.hasComment(section, name)
    return cfg.comments[section] and cfg.config[section][name] ~= nil
end

function cfg.get(section, name)
    return cfg.config[section] and cfg.config[section][name] or nil
end

function cfg.read(filePath, defaultConfig)
    if fs.exists(filePath) then
        cfg.config = config.read(filePath)
        return
    else
...
function cfg.serialize()
    local out = {}
    local outLen = 0

    local function writeVal(k, v, comment)
        if comment then
            outLen = outLen + 1
            out[outLen] = "; " .. comment
        end
        outLen = outLen + 1
        out[outLen] = tostring(k) .. "=" .. (type(v)~="table" and tostring or table.concat)(v, ",") --The comma is only for table.concat.
    end

    local function writeSection(k)
        outLen = outLen + 1
        out[outLen] = "["..tostring(k).."]"
    end
    ...
    for k, v in pairs(cfg.config) do
        if type(v)=="table" then
            ...
            for k2, v2 in pairs(v) do
                writeVal(k2, v2, cfg.comments[k] and cfg.comments[k][k2] or nil)
            end
        else
            writeVal(k, v, cfg.comments[k])
        end
    end
    return out
end

function cfg.toString()
    return table.concat(cfg.serialize(), "\n")
end

function cfg.write(filePath)
    local f = fs.open(filePath, "w")
    for _, v in pairs(cfg.serialize()) do
        f.write(v)
        f.write"\n"
    end
    f.close()
